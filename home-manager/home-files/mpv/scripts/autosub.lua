...
-- NOTE: credentials must NOT be hardcoded here — this file is copied into the
-- world-readable Nix store and tracked in git. Instead they are injected via
-- environment variables, set in ~/.config/mpv/secrets.env which the mpv
-- wrapper sources at launch (see mpv.nix). To use a provider, export
-- <PROVIDER>_USER / <PROVIDER>_PASSWORD there.
local logins = {
...
do
    local osub_user = os.getenv('OPENSUBTITLES_USER')
    local osub_pass = os.getenv('OPENSUBTITLES_PASSWORD')
    if osub_user and osub_pass then
        logins[#logins + 1] = { '--opensubtitles', osub_user, osub_pass }
    end
end
--=============================================================================
-->>    ADDITIONAL OPTIONS:
...
mp.add_key_binding('p', 'download_subs2', download_subs2)
